...
class Dialog( QDialog, Ui_Dialog ):
	...
	def voronoi_polygons( self ):
		...
		if False:
			...
			self.voronoi_tesselation( writer, vprovider, points, bounds )


	def voronoi_tesselation( self, writer, provider, points, bounds ):
		coords, equation, edges = voronoi.computeVoronoiDiagram( points )
		...
		for index,v1,v2 in edges:
			...
			if index == 3:
				break
		rect = [ 
		QgsPoint( bounds.xMinimum(), bounds.yMinimum() ),
		QgsPoint( bounds.xMaximum(), bounds.yMinimum() ),
		QgsPoint( bounds.xMaximum(), bounds.yMaximum() ),
		QgsPoint( bounds.xMinimum(), bounds.yMaximum() ),
		QgsPoint( bounds.xMinimum(), bounds.yMinimum() ) ]
