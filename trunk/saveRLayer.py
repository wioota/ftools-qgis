	...
	if not outName.isEmpty():
		...
		if fileCheck.exists():
			...
			if not outName.isEmpty():
				...
				try:
					exec "r(''' writeOGR(obj = " + unicode(mlayer) + ", dsn = '" + unicode(filePath) + "', layer = '" + unicode(fileName) + "', driver = '" + unicode(Driver) + "') ''')"
					returnText += " Done\n"
			...
			if not outName.isEmpty():
				...
				try:
					if Driver == "AAIGrid" or Driver == "JPEG2000" or Driver == "PNG" or Driver == "USGSDEM":
						exec "r(''' saveDataset(dataset = copyDataset(create2GDAL(dataset = " + unicode(mlayer) + ", type = 'Float32'), driver = '" + unicode(Driver) + "'), filename = '" + unicode(filePath) + "') ''')"
					else:
						exec "r(''' writeGDAL(dataset = " + unicode(mlayer) + ", fname = '" + unicode(filePath) + "', drivername = '" + unicode(Driver) + "', type = 'Float32') ''')"
					returnText += " Done\n"
